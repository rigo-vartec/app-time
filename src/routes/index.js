...
router.get("/user/logged/:rol", async (req, res) => {
  ...
  const maquina = await db.query("SELECT * FROM averias_cerradas WHERE user_id=? order by id desc LIMIT 1", [req.user.id]);
...
router.post("/incidencia/add", async (req, res) => {
  ...
  const {
    creation_date, turno, num_machine, start_date, start_time, step, page } = req.body;
  const newave = {
    creation_date,
    turno,
    num_machine,
...
router.post("/averia/add", async (req, res) => {
  let username = req.user.username;
  const {
    creation_date, turno, start_date, start_time, step, page } = req.body;
  const newave = {
    creation_date,
    turno,
...
router.post("/arrival/validate/:id", async (req, res) => {
  ...
  const row = await db.query(
    'SELECT * FROM users WHERE username = ? AND rol="mantenimiento"', [username]);
  if (row.length > 0) {
    const id1 = row[0];
...
router.get('/averias/operador/calidad/buscar', async (req, res) => {
  const averia_calidad = await db.query("SELECT * FROM averias_cerradas WHERE estatus_mantto='aprobado' AND estatus_calidad='indefinido'", [req.user.id]);
